...
export default function DashboardPage() {
  ...
  const openCreate = () => { setEditing(null); setCategoryMode('existing'); setForm({ ...blankPost, products: [{ ...blankProduct }] }); setNotice(''); };
  const openEdit = (post: Post) => { setEditing(post); setCategoryMode('existing'); setForm({ title: post.title, slug: post.slug, category: post.category, accent_color: post.accent_color, intro_text: post.intro_text, cover_image: post.cover_image, products: post.products.length ? post.products : [{ ...blankProduct }], conclusion_text: post.conclusion_text }); setNotice(''); };
        ...
        <div className="mt-10 grid gap-10 xl:grid-cols-[.85fr_1.15fr]">
          ...
          <section className="border border-border bg-card p-5 sm:p-7"><div className="flex items-end justify-between border-b border-border pb-4"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-primary">{editing ? 'Edit story' : 'New story'}</p><h2 className="mt-1 font-display text-3xl">{editing ? 'Refine the room' : 'Put it on paper'}</h2></div>{editing && <button type="button" onClick={openCreate} data-testid="button-cancel-edit" className="inline-flex items-center gap-1 text-xs uppercase tracking-[.12em] text-muted-foreground hover:text-primary"><X size={15} /> Clear</button>}</div>
            <form onSubmit={submit} className="mt-6 space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="field-label">Slug</span><input required value={form.slug} onChange={(event) => updateField('slug', event.target.value)} data-testid="input-post-slug" className="field-input" placeholder="room-with-somewhere-to-go" /></label><div><span className="field-label">Category</span>{categoryMode === 'existing' ? <select required value={form.category} onChange={(event) => { if (event.target.value === '__new__') { setCategoryMode('new'); updateField('category', ''); } else { updateField('category', event.target.value); } }} data-testid="select-post-category" className="field-input"><option value="" disabled>{categories.length ? 'Choose a category' : 'Loading categories...'}</option>{categories.map((category) => <option key={category.name} value={category.name}>{category.name}</option>)}<option value="__new__">+ Create a new category</option></select> : <div className="flex items-start gap-2"><input required autoFocus value={form.category} onChange={(event) => updateField('category', event.target.value)} data-testid="input-post-category" className="field-input" placeholder="e.g. Reading Nooks" /><button type="button" onClick={() => { setCategoryMode('existing'); updateField('category', categories[0]?.name ?? ''); }} data-testid="button-use-existing-category" className="mt-2 shrink-0 text-[10px] font-bold uppercase tracking-[.12em] text-primary hover:text-foreground">Use existing</button></div>}</div></div>
